...
protected:
    ...
    {
        mTestPoints = std::make_shared<Model::Point3DList>();
        mTestPoints->reserve(1000);
        for (double x = 0; x < 100; x += 0.1)
        {
            double y = 5 * std::sin(0.01 * x) + 10 * std::cos(0.1 * x) + 5;
            double z = 0.1 * x;
            mTestPoints->push_back(std::make_shared<Model::Point3D>(x, y, z));
        }
...
{
    double ZERO = 1e-6;
    double distance = Model::DouglasPeucker::pointToPointDistance(mPoint1,
                                                                  mPoint2);
    EXPECT_NEAR(distance, 10, ZERO);
    distance = Model::DouglasPeucker::pointToPointDistance(mPoint1, mPoint3);
    EXPECT_NEAR(distance, 6, ZERO);
    distance = Model::DouglasPeucker::pointToPointDistance(mPoint2, mPoint3);
...
{
    double ZERO = 1e-6;
    double distance = Model::DouglasPeucker::pointToLineDistance(mPoint1,
                                                                 mPoint2,
                                                                 mPoint3);
    EXPECT_NEAR(distance, 6, ZERO);
    distance = Model::DouglasPeucker::pointToLineDistance(mPoint2,
                                                          mPoint1,
                                                          mPoint3);
    EXPECT_NEAR(distance, 8, ZERO);
    distance = Model::DouglasPeucker::pointToLineDistance(mPoint3,
                                                          mPoint1,
                                                          mPoint2);
...
{
    double ZERO = 1e-6;
    std::pair<std::uint32_t, double> fp = Model::DouglasPeucker
                                          ::getFarthestPoint(mTestPoints, 0, 1000);
    EXPECT_EQ(fp.first, 295);
    EXPECT_NEAR(fp.second, 14.040098356989594, ZERO);
    fp = Model::DouglasPeucker::getFarthestPoint(mTestPoints, 0, 295);
    EXPECT_EQ(fp.first, 74);
    EXPECT_NEAR(fp.second, 2.006942452711151, ZERO);
    fp = Model::DouglasPeucker::getFarthestPoint(mTestPoints, 295, 1000);
    EXPECT_EQ(fp.first, 626);
    EXPECT_NEAR(fp.second, 19.294119951184758, ZERO);
    fp = Model::DouglasPeucker::getFarthestPoint(mTestPoints, 295, 626);
    EXPECT_EQ(fp.first, 378);
    EXPECT_NEAR(fp.second, 2.6619359453332816, ZERO);
    fp = Model::DouglasPeucker::getFarthestPoint(mTestPoints, 626, 1000);
...
{
    double ZERO = 1e-6;
    Model::Point3DListPtr points = Model::DouglasPeucker
                                   ::Simplify(mTestPoints, 5);
    ...
    points = Model::DouglasPeucker::Simplify(mTestPoints, 2);
