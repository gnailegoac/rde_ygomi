...
void NurbsCurveFit(int n, int p, int m, double* U, double* u, cv::Point3d* Q, cv::Point3d* P)
{
    cv::Mat A(m - 1, n - 1, CV_64F);
    ...
    for(int k = 1; k < m; k++)
    {
        ...
        for(int j = 0; j < n - 1; j++)
        {
            ...
            R_mat.at<double>(j, 0) = R_mat.at<double>(j, 0) + N[j + 1] * R[k].x;
            R_mat.at<double>(j, 1) = R_mat.at<double>(j, 1) + N[j + 1] * R[k].y;
            R_mat.at<double>(j, 2) = R_mat.at<double>(j, 2) + N[j + 1] * R[k].z;
        }
    }

    cv::Mat Rx(n - 1, 1, CV_64F);
    cv::Mat Ry(n - 1, 1, CV_64F);
    cv::Mat Rz(n - 1, 1, CV_64F);
    cv::Mat Px(n - 1, 1, CV_64F);
    cv::Mat Py(n - 1, 1, CV_64F);
    cv::Mat Pz(n - 1, 1, CV_64F);
    Rx = R_mat.col(0);
    Ry = R_mat.col(1);
    Rz = R_mat.col(2);
    cv::Mat B = A.t() * A;
