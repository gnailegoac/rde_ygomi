...
uMin(aMin), uMax(aMax)
{
    for (const auto& pt : aCtrlPts) vecCtrlPoints.push_back(pt);
    for (const auto& t : aKnots) vecKnots.push_back(t);
...
{
    for (const auto& pt : nurbs_curve.GetCtrlPoints()) vecCtrlPoints.push_back(pt);
    for (const auto& t : nurbs_curve.GetKnots()) vecKnots.push_back(t);
...
double NURBS::GetLengthByPoint(double aMin, double aMax) const
{
    ...
    if (DOUBLE_EQ(aMin, aMax))  return 0.0;
...
{
    double length = 0;
    for (const auto iter : vecCurve) length += get_poly2_Len(&iter, iter.t_min, iter.t_max);
    return length;
}
